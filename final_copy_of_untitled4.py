...
# 1. Sélection des colonnes Lean
lean_columns = df.filter(regex=r'^Lean')

# 2. Calcul du nombre d'entreprises utilisant chaque méthode Lean
...
# 4. Trier par niveau d'utilisation (du plus utilisé au moins utilisé)
...
# 1. Sélection des colonnes Industrie 4.0
...
mapping = {
    'électrique/électronique/électroménager': 'Électronique et Énergie',
    'câblage': 'Électronique et Énergie',
    'Energitique': 'Électronique et Énergie',
    'Aéronautique': 'Transport et Mobilité',
    'Automobile': 'Transport et Mobilité',
    'Transport': 'Transport et Mobilité',
    'Agroalimentaire': 'Agro-Pharma',
    'Pharmaceutique': 'Agro-Pharma',
    'mécanique et métallurgie': 'Industrie Lourde',
    'textile et habillement': 'Industrie Lourde',
    'Consulting': 'Services et Autres',
    'Développement embarqué': 'Services et Autres',
    'Autres': 'Services et Autres'
}


# Application du mapping
df['Secteur Regroupé'] = df['Secteur industriel'].map(mapping)

print(df)

df.head()

# prompt: distribution du Secteur Regroupé

# Get the value counts for the new grouped column
grouped_sector_distribution = df['Secteur Regroupé'].value_counts()

print("\nDistribution des valeurs pour 'Secteur Regroupé':")
print(tabulate(grouped_sector_distribution.reset_index(), headers=['Secteur Regroupé', 'Count'], tablefmt='psql'))

# Visualize the distribution of the grouped sectors
plt.figure(figsize=(10, 6))
sns.countplot(data=df, y='Secteur Regroupé', order=grouped_sector_distribution.index)
plt.title("Distribution des secteurs industriels regroupés")
plt.xlabel("Count")
plt.ylabel("Secteur Regroupé")
plt.tight_layout()
plt.show()

# Create 'Lean_Juste à temps' column
...
x_dim = ["Technologies", "Opérations"]
y_dim = ["Stratégie - Leadership", "Supply Chain"]
...
fig = px.scatter(
    ...
    labels={"x": "Axe Technico-opérationnel", "y": "Axe Engagement Stratégique"},
...
fig.update_layout(
    title="🧭 Positionnement de l’entreprise sur les axes de maturité Lean 4.0",
    xaxis_title="Axe Technico-opérationnel (Technologies & Opérations)",
    yaxis_title="Axe Engagement Stratégique (Leadership & Supply Chain)",
...
# Supposons que pca est déjà entraîné et que features est la DataFrame des features d'origine
components = pca.components_
feature_names = features.columns

# Créer un DataFrame des composantes, valeurs absolues
...
# DataFrame clair avec les contributions en pourcentage arrondies à 2 décimales
df_contrib = pd.DataFrame(pct_contrib, columns=feature_names,
                          index=[f'PC{i+1}' for i in range(pca.n_components_)])

# Trier chaque ligne du DataFrame par ordre décroissant et afficher
...
# Créer un mapping : sous-dimension ➝ dimension
...
# Affichage trié pour chaque composante principale
...
# Create 'Lean_Juste à temps' column
df['Lean_Juste à temps'] = df['Opérations - Juste-à-temps (JAT)'].apply(lambda x: 1 if x in [4, 5] else 0)
#df['Tech_IoT'] = df['Technologies - Connectivité et gestion des données'].apply(lambda x: 1 if x in [4, 5] else 0)
...
# prompt: Appliquer dummies to 'Secteur Regroupé'

# Apply one-hot encoding to 'Secteur Regroupé'
df = pd.get_dummies(df, columns=['Secteur Regroupé'], prefix='SecteurREG')

# Display the first few rows to see the new dummy columns
print("\nDataFrame after applying one-hot encoding to 'Secteur Regroupé':")
print(df.head())

# prompt: Ajouter une variable qui regroupe les colonnes dummies du secteur regroupé

# Variable to group the dummy columns for 'Secteur Regroupé'
secteur_dummies = [col for col in df.columns if col.startswith('SecteurREG_')]

# prompt: Fixer le types des variables dans colonnes à integer
...
# Liste des colonnes Lean et Tech spécifiques que tu veux garder
lean_cols = [
    'Lean_5S',
    'Lean_Value Stream Mapping (VSM)',
    'Lean_Kaizen',
    'Lean_Méthode TPM / TRS',
    'Lean_QRQC',
    'Lean_Kanban',
    'Lean_Takt Time',
    'Lean_6 sigma',
...
# Filtrer celles qui existent réellement dans le DataFrame
...
# prompt: Merci d'afficher les colonnes des méthodes Lean par pairs selon la force de corrélation   entre les méthodes Lean
...
print("\nCorrélation entre les méthodes Lean (par paires, triée par force de corrélation):")
...
# prompt: Merci d'afficher les colonnes des méthodes Lean par pairs selon la force de corrélation   entre les technologies industrie 4.0
...
print("\nCorrélation entre les technologies Industrie 4.0 (par paires, triée par force de corrélation):")
...
# prompt: Merci d'afficher les colonnes des méthodes Lean et technologies industrie 4.0 par pairs selon la force de corrélation
...
print("\nCorrélation entre les méthodes Lean et les technologies Industrie 4.0 (par paires, triée par force de corrélation):")
...
# prompt: === 📈 Visualizing Average Survey Scores per Cluster ===
# === 📈 Visualizing Average Lean Method and Tech Presence per Cluster ===
...
# 4. Normalisation en poids (%) pour interprétation
feature_importance = (inter_cluster_variance / inter_cluster_variance.sum()) * 100

# 5. Affichage trié des poids par ordre décroissant
...
# 3. Variance inter-cluster : écart des clusters par rapport à la moyenne globale
...
# 5. Affichage des poids triés
weights_sorted = weights.sort_values(ascending=False).round(4)
print("📊 Poids normalisés des sous-dimensions (importance relative dans le KMeans) :\n")
...
# prompt: cammabère de colonne 'taille_categorie'
...
if 'taille_categorie' in df.columns:
    ...
    print(tabulate(taille_categorie_counts.reset_index(), headers=['Catégorie de Taille', 'Count'], tablefmt='psql'))
else:
    print("'taille_categorie' column not found in the DataFrame.")

# prompt: i want to group 'Secteur industriel' selon :
# 'Agroalimentaire': 'Industrie légère',
#     'Automobile': 'Industrie lourde',
#     'Aéronautique': 'Industrie lourde',
#     'électrique/électronique/électroménager': 'Industrie légère',
#     'textile et habillement': 'Industrie légère',
#     'mécanique et métallurgie': 'Industrie lourde',
#     'Pharmaceutique': 'Industrie lourde',
#     'Développement embarqué': 'Service et Technologie',
#     'Energitique': 'Service et Technologie',
#     'Consulting': 'Service et Technologie',
#     'Transport': 'Service et Technologie',
#     'câblage': 'Industrie légère',
#     'Autres': 'Divers'
...
if 'df' in locals() and not df.empty:
  ...
  if 'Secteur industriel' in df.columns:
    df['Secteur_Groupe'] = df['Secteur industriel'].map(secteur_mapping).fillna('Autre/Non défini')
    ...
    unmapped_sectors = df[df['Secteur_Groupe'] == 'Autre/Non défini']['Secteur industriel'].unique()
